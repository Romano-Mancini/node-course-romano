...
import { getSuggestions } from "./handlers/llm.suggestion.handler";
...
export class FridgeController {
	...
	@ApiOperation({
		...
		summary: "User gets suggestions generated by an LLM",
	})
	@ApiResponse({
		description: "Suggestions correctly fetched.",
	})
	@HttpCode(HttpStatus.OK)
	async getSuggestions(@Req() req: any): Promise<RecipeBody[]> {
		const res = getSuggestions(req.user.userId);
		return res;
	}
